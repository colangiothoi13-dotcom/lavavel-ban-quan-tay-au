...
- Nhắn tin riêng với quản trị viên theo thời gian thực.
...
- Xem toàn bộ cuộc hội thoại và trả lời tin nhắn của khách hàng.
...
Chạy các lệnh sau trong ba terminal riêng:
...
### Cấu hình chat thời gian thực

Sau khi sao chép `.env.example`, bảo đảm `.env` đang dùng Reverb cho broadcasting. Các giá trị dưới đây chỉ là mẫu local; không đưa secret thật vào README:
...
REVERB_PORT=8080
REVERB_SCHEME=http
VITE_REVERB_APP_KEY=local-chat-key
VITE_REVERB_HOST=127.0.0.1
VITE_REVERB_PORT=8080
VITE_REVERB_SCHEME=http
```

Tài khoản chưa đăng nhập vẫn thấy mục “Nhắn tin” nhưng sẽ được yêu cầu đăng nhập khi mở mục này. Mỗi tài khoản có một cuộc hội thoại riêng được lưu vào database; tin nhắn gửi khi admin offline sẽ xuất hiện khi admin đăng nhập. Admin xem được tất cả cuộc hội thoại, còn trạng thái online/bận của admin được xác định bằng heartbeat.
