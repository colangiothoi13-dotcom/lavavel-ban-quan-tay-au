...
{
    private const FILTERABLE_SIZES = ['S', 'M', 'L', 'XL'];
    ...
    public function home(Request $request)
    {
        ...
        if (! $request->user()?->isAdmin() && in_array($request->input('size'), self::FILTERABLE_SIZES, true)) {
    ...
    public function checkout(Request $request)
    {
        ...
        $selectedIds = is_array($selectedIds)
            ? array_values(array_filter($selectedIds, static fn ($id): bool => (string) $id !== ''))
            : [];
    ...
    public function placeOrder(Request $request): RedirectResponse
    {
        $data = $request->validate([
            ...
            'payment_method' => ['required', 'in:cash,bank_transfer,international_transfer,momo,momo_atm,momo_cc,cod'],
        ...
        DB::transaction(function () use ($items, $request, $data, $subtotal, $shippingFee, &$order): void {
            ...
            $order = $request->user()->orders()->create([
                ...
                'payment_expires_at' => in_array($paymentMethod, [Order::PAYMENT_METHOD_MOMO, Order::PAYMENT_METHOD_MOMO_ATM, Order::PAYMENT_METHOD_MOMO_CC], true)
    ...
    private function normalizePaymentMethod(string $paymentMethod): string
    {
        return match ($paymentMethod) {
            'cod', 'cash' => Order::PAYMENT_METHOD_CASH,
            'momo' => Order::PAYMENT_METHOD_MOMO,
            'momo_atm' => Order::PAYMENT_METHOD_MOMO_ATM,
            'momo_cc' => Order::PAYMENT_METHOD_MOMO_CC,
