...
(() => {
    ...
    const renderSuggestions = (items) => {
        ...
        suggestions.innerHTML = items.map(item => `<li><button type="button">${item.label}</button></li>`).join('');
        suggestions.querySelectorAll('button').forEach((button, index) => {
            button.addEventListener('click', () => {
                const selected = items[index];
                street.value = selected.label;
                closeSuggestions();
                showPosition(selected.latitude, selected.longitude, 18);
                say('Đã chọn địa chỉ trên bản đồ. Bạn có thể kéo ghim để chỉnh vị trí.');
            });
        });
    ...
    street.addEventListener('input',()=>{clearTimeout(addressMapTimer);closeSuggestions();const specificAddress=street.value.trim();if(specificAddress.length<2||!ward.value)return;addressMapTimer=setTimeout(async()=>{try{const params = new URLSearchParams({q:`${specificAddress}, ${ward.selectedOptions[0].text}, ${district.selectedOptions[0].text}, ${province.selectedOptions[0].text}, Việt Nam`, specific_address: specificAddress, ward: ward.selectedOptions[0].text, district: district.selectedOptions[0].text, province: province.selectedOptions[0].text}); const response = await fetch(`${ghnUrls.search}?${params}`, {headers:{Accept:'application/json'}}); const payload = await response.json().catch(() => ({results: []})); const results = response.ok ? (payload.results || []) : []; if (!results.length) { say('Không tìm thấy địa chỉ phù hợp. Hãy nhập tên đường hoặc bấm vào bản đồ để ghim chính xác.', true); return; } renderSuggestions(results); say('Chọn một địa chỉ trong danh sách gợi ý.'); } catch { say('Không thể tìm địa chỉ. Hãy thử lại hoặc bấm trực tiếp vào bản đồ.', true); }},500);});
    ...
    document.querySelectorAll('[data-close-address]').forEach(button=>button.addEventListener('click',()=>{modal.classList.remove('open'); closeSuggestions();}));
    document.addEventListener('click', event => { if (!event.target.closest('.address-suggestions') && event.target !== street) closeSuggestions(); });
    document.addEventListener('keydown',event=>{if(event.key==='Escape')modal.classList.remove('open');closeSuggestions();});
    loadProvinces();
})();
</script>
@endsection
