...
class MomoPaymentService
{
    ...
    public function __construct(
        ...
        $loadedConfig = $config ?: config('services.momo', []);

        if (! empty($loadedConfig['endpoint'])) {
            $endpoint = parse_url((string) $loadedConfig['endpoint']);
            if ($endpoint !== false && ! empty($endpoint['scheme']) && ! empty($endpoint['host'])) {
                $endpointBaseUrl = $endpoint['scheme'].'://'.$endpoint['host']
                    .(isset($endpoint['port']) ? ':'.$endpoint['port'] : '');
                $loadedConfig['base_url'] = ($loadedConfig['base_url'] ?? null) ?: $endpointBaseUrl;
                $loadedConfig['create_endpoint'] = ($loadedConfig['create_endpoint'] ?? null)
                    ?: (($endpoint['path'] ?? '/').(isset($endpoint['query']) ? '?'.$endpoint['query'] : ''));
            }
        }

        $loadedConfig['base_url'] = ($loadedConfig['base_url'] ?? null) ?: $defaults['base_url'];
        $loadedConfig['create_endpoint'] = ($loadedConfig['create_endpoint'] ?? null) ?: $defaults['create_endpoint'];

        $this->config = array_replace($defaults, $loadedConfig);
    }
    ...
    public function createPayment(Order $order, string $successRoute, string $failRoute): array
    {
        ...
        $requestType = match ($order->payment_method) {
            Order::PAYMENT_METHOD_MOMO_ATM => 'payWithATM',
            Order::PAYMENT_METHOD_MOMO_CC => 'payWithCC',
    ...
    {
        return Http::asJson()->timeout((int) ($this->config['timeout'] ?? 15));
    }
}
