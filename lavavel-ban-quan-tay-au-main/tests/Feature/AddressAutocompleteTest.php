...
class AddressAutocompleteTest extends TestCase
{
    ...
    public function test_address_search_keeps_results_when_ward_name_partially_matches(): void
    {
        Http::fake([
            'photon.komoot.io/*' => Http::response([
                'features' => [[
                    'geometry' => ['coordinates' => [106.7009, 10.7769]],
                    'properties' => ['name' => '41A', 'city' => 'Hồ Chí Minh', 'county' => 'Thành phố Hồ Chí Minh'],
                ]],
            ]),
        ]);
        $user = User::factory()->create();
    ...
    public function test_shop_home_contains_live_product_search_suggestions(): void
    {
        $this->get(route('shop.home'))
            ->assertOk()
            ->assertSee('id="product-search-input"', false)
            ->assertSee('id="product-search-suggestions"', false)
            ->assertSee('shop.products.suggestions', false);
    }

    public function test_address_page_contains_live_address_suggestions_ui(): void
    {
        $user = User::factory()->create();
        ...
        $this->actingAs($user)
            ...
            ->assertOk()
            ->assertSee('id="address-suggestions"', false)
            ->assertSee('aria-controls="address-suggestions"', false);
    }
