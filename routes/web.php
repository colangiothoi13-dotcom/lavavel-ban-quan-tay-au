...
Route::prefix('nhan-tin')->middleware('auth')->group(function () {
    ...
    Route::post('/messages', [ChatController::class, 'userSend'])
        ...
        ->name('chat.user.messages.store');
    Route::post('/read', [ChatController::class, 'userRead'])->name('chat.user.read');
    Route::get('/admin-status', [ChatController::class, 'userAdminStatus'])->name('chat.user.admin-status');
});
...
Route::prefix('admin')->middleware(['auth', 'admin'])->group(function () {
    Route::get('/nhan-tin', [ChatController::class, 'adminIndex'])->name('chat.admin.index');
    Route::post('/nhan-tin/presence', [ChatController::class, 'adminPresence'])->name('chat.admin.presence');
    Route::get('/nhan-tin/conversations', [ChatController::class, 'adminConversations'])->name('chat.admin.conversations');
    Route::get('/nhan-tin/{conversation}/messages', [ChatController::class, 'adminMessages'])->name('chat.admin.messages');
    Route::post('/nhan-tin/{conversation}/messages', [ChatController::class, 'adminSend'])
        ->middleware('throttle:30,1')
        ->name('chat.admin.messages.store');
    Route::post('/nhan-tin/{conversation}/read', [ChatController::class, 'adminRead'])->name('chat.admin.read');
});
